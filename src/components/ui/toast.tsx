...
>(({ className, variant, onOpenChange, style, ...props }, ref) => {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const diffX = current.x - startPos.current.x;
    const diffY = current.y - startPos.current.y;

    // Allow swipe left, right, down (ignore strong upward swipes)
    if (diffY < -20) return;

    setOffset({ x: diffX, y: Math.max(0, diffY) });
    
    // Fade out based on distance
    const distance = Math.sqrt(diffX * diffX + diffY * diffY);
    const newOpacity = Math.max(0, 1 - distance / 250);
  ...
  const handleTouchEnd = () => {
    ...
    const threshold = 75;
    if (Math.abs(offset.x) > threshold || offset.y > threshold) {
      isDismissed.current = true;
      setOpacity(0);
      
      // Animate throw out
      if (Math.abs(offset.x) > threshold) {
         setOffset(prev => ({ ...prev, x: prev.x + (prev.x > 0 ? 400 : -400) }));
      } else {
         setOffset(prev => ({ ...prev, y: prev.y + 400 }));
      }
